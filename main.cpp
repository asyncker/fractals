...
inline double inv_sqrt(double x) {
	union { double d; unsigned long long i; } u; u.d = x;
...
inline double fast_pow(double a, double b) {
	...
	if (b == -0.5) return inv_sqrt(a);
	if (b == -1.5) return 1.0 / a * inv_sqrt(a);
	return 1.0 / pow(a, -b);
}
...
inline int bounce(int x, int min, int max) {
...
public:
	...
	inline void save_as(const std::string& filename) {
		...
		int extra_bytes = 4 - ((width * 3) % 4), iter = 2;
...
inline void mandelbrot(double* arrays, int fractal_index, double z_x, double z_i, double c_x, double c_i, double factor, double power, double range, double inverse, double mult_power, int ismandel, int width, int height, int max_iteration, long max_sample, int is_mandelbulb) {
	...
	double c_real, c_imag, z_real, z_imag, z_real_power = 0.0, z_imag_power = 0.0, z_power_sum = 0.0, z_atan2, power_sum_invert, z_imag_prev = 0.0, z_real_prev = 0.0;
	double z_cos_real, z_cos_imag, z_right_real, z_right_imag, PI = 3.14159265359, kx, ky, z_power, z_power_invert;
	...
	double log_value = power > 2 ? 1.0 / fast_log(fabs(power)) : 1.44269504088896, smooth_value = 0.0, nu = 0.0;
	double real_power = power * 0.5 * mult_power, power_minus_one = power - 1.0, real_newton_power = (power - 1.0) * 0.5 * mult_power, half_log_value = log_value * 0.5;
	int is_power_two = power == 2.0 && mult_power == 1.0, is_power_one = power == 1.0 && mult_power == 1.0, index = 0, n = 1;
	if (fractal_index == 9) { range = 1e+21; }
	...
	for (int y = 0; y < height; y++) {
		for (int x = 0; x < width; x++) {
			c_real = ismandel ? dx * x + x_min : c_x;
			c_imag = ismandel ? dy * y + y_min : c_i;
			if (inverse != 0.0) {
			...
			z_real = ismandel ? c_real : dx * x + x_min;
			z_imag = ismandel ? c_imag : dy * y + y_min;
...
inline void mandel4d(double* arrays, int fractal_index, double z_x, double z_y, double cx, double cy, double factor, double power, double range, double inverse, double mult_power, int ismandel, int width, int height, int max_iteration, long max_sample, double min_dist, double rot_x, double rot_z, int raymarch_iterations, double phi_shift, int formula, double cz, double cw, double zz, double zw, int is_mandelbulb) {
	...
	for (int y = 0; y < height; y++) {
		...
		for (int x = 0; x < width; x++) {
			...
			double rd_z = inv_sqrt(uv_x * uv_x + uv_y * uv_y + 1.0);
					...
					for (int j = 1; j < max_iterations; j++) {
						...
						if (formula == 0) {
							if (is_power_two) {
								...
								z_z_prev = 2.0 * z_z * fast_sqrt(zxy2);
								z_x_prev = (zx2 - zy2) * a;
								dr = r * 2.0 * dr + 1.0;
							}
							else {
						...
						if (fractal_index < 3) {
							if (fractal_index == 1) { // tricorn
								z_y_prev = -z_y_prev; // add z_z_prev = -z_z_prev;
							}
							z_x = z_x_prev;
							z_y = z_y_prev;
							z_z = z_z_prev;
...
inline void buddhabrot(double* arrays, int fractal_index, double z_x, double z_i, double c_x, double c_i, double factor, double power, double range, double inverse, double mult_power, int ismandel, int width, int height, int max_iteration, long max_sample, int is_mandelbulb) {
	double x_set = ismandel ? c_x : z_x;
	double i_set = ismandel ? c_i : z_i;
	double x_min = x_set - 1.5 * factor;
	double x_max = x_set + 1.5 * factor;
	...
	double nx_factor = 1.0 / x * width;
	double ny_factor = 1.0 / y * height;
	double c_real, c_imag, z_real, z_imag, z_real_power = 0.0, z_imag_power = 0.0, z_power_sum = 0.0, z_atan2, power_sum_invert, z_imag_prev = 0.0, z_real_prev = 0.0;
	double z_cos_real, z_cos_imag, z_right_real, z_right_imag, PI = 3.14159265359, kx, ky, z_power, z_power_invert;
	double z_temp_real = 0.0, z_temp_imag = 0.0, z_real2 = 0.0, z_imag2 = 0.0, p_real = -0.5, p_imag = 0.0, z_real_minus_tmp_real_power = 0.0, z_imag_minus_tmp_imag_power = 0.0;
	double dx, dy, mult_distrub = 1.0 / (double)0xFFFFFFFF, z_old_real = 0.0, z_old_imag = 0.0;
	double real_power = power * 0.5 * mult_power, power_minus_one = power - 1.0, real_newton_power = (power - 1.0) * 0.5 * mult_power;
	int is_power_two = power == 2.0 && mult_power == 1.0, is_power_one = power == 1.0 && mult_power == 1.0, index = 0, escape_newton = 0, area = width * height, is_escape = 0;
	if (fractal_index == 6) { range = pow(range, 21.0); }
	for (long i = 0; i < max_sample; i++) {
		z_real2 = z_imag2 = 0.0;
		current = (166421U * current + 1054222352U) % 0xFFFFFFFF;
		dx = current * mult_distrub;
		current = (1664525U * current + 1013904223U) % 0xFFFFFFFF;
		dy = current * mult_distrub;
		c_real = ismandel ? (dx * x + x_min) : c_x;
		c_imag = ismandel ? (dy * y + y_min) : c_i;
		if (inverse != 0.0) {
			power_sum_invert = 1.0 / (c_real * c_real + c_imag * c_imag) * inverse;
		...
		for (int n = 1; n < max_iteration; n++) {
			...
			if (is_escape) {
				...
				index = (int)((z_real - x_min) * nx_factor) + (int)((z_imag - y_min) * ny_factor) * width;
				if (index < area) {
					arrays[index]++;
				}
			}
...
inline vec3 hue(double inp_x, double inp_y, double inp_z, double H) {
	...
	ret.z = (0.299 - 0.299 * U + 1.250 * W) * inp_x + (0.587 - 0.587 * U - 1.050 * W) * inp_y + (0.114 + 0.886 * U - 0.203 * W) * inp_z;
	return ret;
}
...
int main() {
	...
	double factor_mandelbrot = 1.5, factor_julia = 1.5, mult_sample = 8.0, mult_light = 0.0, light = 1.7689, val = 0.0, render_light_mult = 1.5;
	...
	std::string row, repeats_button;
	std::vector<std::string> push_button, sequence_button;
	while (1) {
				...
				for (int y = 0; y < height - 1; y++) {
					for (int x = 0; x < width; x++) {
						...
						if (is_mandelbulb || is_mandel4d) {
							...
							r = bounce(val, 0, 255);
							g = bounce(val * 2.0, 0, 255);
							b = bounce(val * 3.0, 0, 255);
						}
						row.push_back('.');
					...
					for (int x = 0; x < width; x++) {
						...
						if (is_mandelbulb || is_mandel4d) {
							...
							if (ot_div != 0.0) {
								...
								val = (clamp(color.x * bright, 0, 255) + clamp(color.y * bright, 0, 255) + clamp(color.z * bright, 0, 255)) * 0.333333333333;
							}
						}
						double value = bounce(val, 0, 255) * 3.0;
						...
						row.push_back(gradient[(int)value % gradient_size]);
					}
					row.push_back('\n');
					print_console(row.c_str(), row.size());
					row.clear();
				...
				for (int y = 0; y < render_height; y++) {
					for (int x = 0; x < render_width; x++) {
						...
						if (is_mandelbulb || is_mandel4d) {
							...
							r = bounce(val * 0.5 * mult_light, 0, 255);
							g = bounce(val * 1.5 * mult_light, 0, 255);
							b = bounce(val * 2.0 * mult_light, 0, 255);
						}
		...
		for (int i = 0; i < repeats_button.size(); i++) {
			...
			if (type_fractal == 1) {
				if (button == 'E') { factor_julia *= 0.95; continue; }
			...
			if (type_fractal == 2) {
				if (button == 'E') { factor_mandelbrot *= 0.95; continue; }
				if (button == 'Q') { factor_mandelbrot *= 1.05; }
				if (button == 'D') { c_x += factor_mandelbrot * 0.05; }
				if (button == 'A') { c_x -= factor_mandelbrot * 0.05; }
				if (button == 'W') { c_y -= factor_mandelbrot * 0.05; }
